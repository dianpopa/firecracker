...
impl MacVTap {
    ...
    pub fn get_device_node(if_name: &str) -> Result<PathBuf> {
        // Must be a char device (but should check after open as well).
        let try_path = |path: PathBuf| {
            path.metadata().and_then(|md| {
        ...
        // If this is run with jailer  /dev/net/<if_name>, then look up via sysfs and try /dev.
        try_path(Path::new("/dev/net").join(if_name))
            .or_else(|_| try_path(Path::new("/dev").join(Self::by_name(if_name)?.tap_name)))
    }
